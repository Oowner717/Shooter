...
export class Dynamo extends Boss {
  ...
  /**
   * One bolt. The routine lives in util.js -- the ARC round's chain lightning
   * is the same picture and was carrying its own worse copy of it -- and this
   * supplies only what is particular to this boss: its colours, and a clock
   * stepped at eighteen a second so the circuit crackles.
   */
  arc(ctx, ax, ay, bx, by, bright, seed, opts = {}) {
    const T = TYPE_BY_ID.dynamo;
    drawBolt(ctx, ax, ay, bx, by, {
      glow: T.glow, hot: '#dceaff', bright, seed, tick: Math.floor(this.t * 18),
      width: opts.width, alpha: opts.alpha, forks: opts.forks, amp: opts.amp,
    });
  }
